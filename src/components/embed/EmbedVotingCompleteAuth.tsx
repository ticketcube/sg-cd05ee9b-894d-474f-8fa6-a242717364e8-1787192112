...
export function EmbedVotingCompleteAuth({ artistCount, pointsToEarn }: EmbedVotingCompleteAuthProps) {
    ...
    const handleOAuthSignIn = async (provider: 'google' | 'apple') => {
        ...
        try {
            ...
            toast.success(`Redirecting to ${provider} sign in...`);
        } catch (error: any) {
            ...
            <div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-indigo-900 flex items-center justify-center p-4">
                <Card className="max-w-md w-full shadow-2xl">
                    ...
                    <CardContent className="space-y-4">
                        {/* Simplified benefits - just points */}
                        <div className="text-center py-2">
                            <div className="flex items-center justify-center gap-2">
                                <Star className="w-6 h-6 text-yellow-500" />
                                <span className="text-2xl font-bold text-gray-900">Earn {pointsToEarn} Points</span>
                            </div>
                        </div>

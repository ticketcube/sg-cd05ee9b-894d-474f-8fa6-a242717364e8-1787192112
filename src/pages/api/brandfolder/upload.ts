...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === "POST") {
        try {
            ...
            const initRes = await fetch(`https://brandfolder.com/api/v4/assets/upload`, {
                method: "POST",
                headers: {
            ...
            return res.status(200).json({ resumableUploadUrl: data.upload_url || data.resumable_upload_url });
        } catch (error: any) {
